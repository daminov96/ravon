...
from apps.rest_api.models import Center, Group, Connection
        ...
        if center_id and group_id:
            ...
            if Center.objects.filter(id=options['center_id']):
                ...
                if Group.objects.filter(center=center, id=options['group_id']):
                    group = Group.objects.filter(center=center, id=options['group_id'])[0]
                    for counter in range(options['counts']):
                        student = CustomUser.objects.create(username=usernameGen("asd JKASDF JASDkad"))
                        Connection.objects.create(user=student, center=center, status='student', is_verified=True)
                        group.students.add(student)
                        group.save()
        elif center_id and not group_id:
            self.stdout.write(f"Creating random users and adding these users to center with id={center_id}")
            if Center.objects.filter(id=options['center_id']):
                center = Center.objects.get(id=options['center_id'])
                for counter in range(options['counts']):
                    student = CustomUser.objects.create(username=usernameGen("asd JKASDF JASDkad"))
